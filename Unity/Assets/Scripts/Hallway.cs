...
//            Gizmos.DrawCube (endPoint,new Vector3(1,1,1));
		...
		{
			var path = Hallway.BresenhamFilled (from: from, to: to, shouldBalanceCorners: shouldBalanceCorners);

			var list = new List<Vector3> (path);
			return list;            
		}

		public static IEnumerable<Vector3> BresenhamFilled (Vector3 from=default(Vector3), Vector3 to=default(Vector3), bool shouldBalanceCorners=false)
		{
			...
			foreach (var point in bresenham) {
				...
				if (lastTwoPoints [0].z != lastTwoPoints [1].z && lastTwoPoints [0].x != lastTwoPoints [1].x) {
					...
					yield return (cornerPoint + from);
				}
				yield return (point + from);
			}
		}
	}
}
